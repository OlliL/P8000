...
/*	There are also several incedental needs, etc, specified
	1,2,3,4	Number of needed registers
	$P	need pairs
	$Q	need quads
	$<	share left
	$>	share right
...
%token STR DEF LPRF RPRF LSHR RSHR GOODCC NOVAL PNEED NHIGHT NLOW QNEED LRES RRES NALL LCOUNT RCOUNT THALF TKLET TXLET
...
nlist	:	/* EMPTY */
	...
	|	nlist PNEED
		{	
			pcount++; needs = (needs & ~PCOUNT)|(pcount << 4); }
	|	nlist QNEED
		{	qcount++;  needs = (needs & ~QCOUNT)|(qcount << 8); }
	|	nlist THALF
		{	rewrite |= NHALF; }
	|	nlist TKLET
		{	rewrite |= RKLET; }
	...
	|	nlist RRES
		{	rewrite |= RRIGHT; }
	|	nlist RREG
		{	if( !(needs&NCOUNT) && !pcount && !qcount ) needs |= $2;
			switch($2) {
