...
# ifndef NODBG
	if( sdebug ) {
		printf( "match tree %d, op %s, p->tn.goal 0x%x\n",
			 (unsigned short)(p-node), opst[pop], p->tn.goal );
		printf("linker typ = 0x%x, rechts = 0x%x, goal = 0x%x\n",tl,tr,goal);
	}
# endif
	for( ; q ; q = q->nextop ){
...
if(sdebug > 3) 
	printf("versuch line %d,ltyp = 0x%x, rtyp = 0x%x, goal = 0x%x\n",
	q->stinline,q->ltype,q->rtype,q->rewrite);	
...
# ifndef NODBG
		if( sdebug ) {
			printf( "	try table entry, line %d, op %s q->rewrite %x\n",
				q->stinline, opst[q->op], q->rewrite );
		}
# endif
		if( !( tyop & q->tyop ) ) {
# ifndef NODBG
...
/*		pstab( S_NARGS, (int)(p->stn.argsize/SZINT));
/*	}
/*# endif
/**/
	if(p->in.op == SHIFTOP && p->in.right->in.op==ICON) {
		typ = p->in.left->tn.type;
		v = p->in.right->tn.lval;
		if(typ & TLONG || typ & TULONG || typ & TPOINT) {
			if(v > 32)
				p->in.right->tn.lval = 32;
		} else if(typ & TCHAR || typ & TUCHAR) {
			if(v > 8)
				p->in.right->tn.lval = 8;
		} else if (v > 16)
				p->in.right->tn.lval = 16;
	}
...
if(odebug>1) printf("op1 rval = %d, op2 rval = %d v = %d c = %d\n", q1->tn.rval,q2->tn.rval,v,c);
			if( q1->tn.rval != (q2->tn.rval  + v))
			{
				if( c ) goto cream;
				continue;
			}
...
# endif
		...
		case 'G': /* offset of an address */
			q1 = getadr(p, &cp);
			if(q1->tn.op != ICON)
				cerror("G but not address constant");
			if(q1->tn.rval != NONAME && q1->tn.name != (char *)0){
				if(q1->tn.rval < 0 && *(q1->tn.name) == 'L')
					q1->tn.rval = 0xa000;
				else
					q1->tn.rval = 0x2000;
			}
		...
		case 'Q': /* offset of an address */
			q1 = getadr(p, &cp);
			if(q1->tn.op != ICON)
				cerror("Q but not address constant");
			if(q1->tn.rval != NONAME && q1->tn.name != (char *)0){
				if(q1->tn.rval < 0 && *(q1->tn.name) == 'L')
					q1->tn.rval = 0x9000;
				else
		...
		case 'P': /* auto and param offset without FP */
			padrput(q1 = getadr( p, &cp) );
			if( sideff ) rmside( q1 );
			continue;

		case 'B':
			/* for BA-adressing in ld-operations */
			nadrput( q1 = getadr(p, &cp));
			continue;

		case 'X':
			/* 'b', 'l' or '\0' - type of operation */
			typ = typlen(p->in.type);
			if(typ == 4)
				PUTCHAR('l');
			else if(p->in.left->tn.op!= REG && typ == 1)
				PUTCHAR('b');
			continue;
...
long pow2[] = 
{
	...
	010000000000L,	020000000000L,	0L,
};
