# include "mfile1.h"
static TWORD orgtleft;
static int convasgflg;
...
/* hier z.B. (int a) -= (unsigned b) => in (unsigned -=) umgebaut und convasgflg gesetzt fuer Ruecknahme, s. vor return(p)
*/
...
# ifndef NODBG
	if( bdebug ) eprint(p);
# endif
	if( convasgflg && asgop(p->in.op) && p->in.left->in.op==CONV )
		p = makety(p,orgtleft,0,(int)orgtleft);
	return(p);
...
# ifndef NODBG
	if( ddebug > 1 ) printf( "chkstr( %s(%d), %ld )\n", stab[i].sname, i, j );
# endif
...
register NODE *p;
{
	...
	*/
	...
	{	/* q1 and q2 still mean left and right */
		...
		if ( BTYPE(t1) == VOID || BTYPE(t2) == VOID ) return;
		combo( "pointer", p );
	}
}
...
# endif
	...
	if( dsc & FIELD )
	{
		...
		p->tn.rval = PKFIELD(dsc&FLDSIZ,((int)(q->offset)%align)&077 );
	}
...
/* 10.4.	p->tn.type = p->fn.csiz = PTRTYPE;
*/
		p->tn.type = p->fn.csiz = INT;
		p->fn.cdim = 0;
		return( clocal(p) );
...
register NODE *p;
{
	...
	*/
	...
	convasgflg = 0;
	orgtleft = t1 = (l=p->in.left)->in.type;
...
register NODE *p;
{
	...
	else
	{
		...
		if( val != p->tn.lval )
/*
		if((p->tn.lval & 0xffff0000) && 
		    (p->tn.lval & 0xffff0000) != 0xffff0000)
*/
			uerror( "warning: constant too big for compiler" );
	}
	tfree( p );
	return(val);
}
...
register NODE *p;
{
	...
	lerror( "operands of %s have incompatible types", opst[o] );
	return( NCVT );
}
...
/*
	...
	Fuer variable Segmentnamen bei Daten geaendert !!!!!!!
*/
	else
	 	printf(locnames[lt], datanm);
	curloc = lt;
	return( temp );
}
# endif
...
register NODE *p;
{
	...
	if( p->in.op == CBRANCH && optype((hp=p->in.left)->in.op) == BITYPE ) {
		if( hp->in.right->in.op==FCON && hp->in.right->fpn.dval==0 ) {
			hp->in.right->in.op = FREE;
			hp->in.right = bcon(0);
		}
		else
		if( hp->in.left->in.op==FCON && hp->in.left->fpn.dval==0 ) {
			hp->in.left->in.op = FREE;
			hp->in.left = bcon(0);
		}
	}
	if( p->in.op == FCON )
