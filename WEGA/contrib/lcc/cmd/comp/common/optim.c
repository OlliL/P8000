...
register adj;
{
	...
	case VPARAM:
		...
		return( 1 );
	case STAR:
		p->in.left = aadjust( p->in.left, (OFFSZ)adj );
		return( 1 );
	default:
		return( 0 );
	}
...
register NODE *p; 
{
	...
	*/
	...
	if( opdebug ) 
	{
		printf("operator %s mit goal 0x%x\n",opst[p->tn.op],p->tn.goal);
	...
	if(  (o == PLUS && ISPTR(r->tn.type)) || 
	((l->tn.op == ICON || l->tn.op == FCON)
		 && (o == MUL || o == OR || o == ER || o == AND))){
		commute(p);
		l = p->in.left;
		r = p->in.right;
	}
/* Hier sollte eine Vereinfachung der Pointerarithmetik eingefuegt werden.
   D.h. Pointer+/-Long ist schneller als (hinterer Teil Pointer)+/-Int
   zu realisieren. 
	if((o==PLUS||o==ASG PLUS||o==MINUS||o==ASG MINUS||o==INCR||o==DECR) &&
		ISPTR(l->tn.type)  && r->tn.type==LONG) {
			if(r->tn.op==CONV && r->in.left->tn.type < LONG) {
				r->tn.op = FREE;
				r = r->in.left;
			} else
...
*/
	...
	if(optype(o)==BITYPE) {
		...
		if((r->tn.op == ICON && r->tn.lval == 0 && !(ISPTR(r->tn.type)))
		|| (r->tn.op == FCON && r->fpn.dval == 0))
			switch(o)
			{
			case PLUS:
			case MINUS:
	...
	asg:			p->in.op = ASSIGN;
			...
			case MOD:
				/* a / 0 */
				werror("divide by 0, convert to const 0");
				goto free;
	...
	switch(o)
	{
		...
		case ASG ER:
			/* a ^ a and a ^= a */
			if((optype(l->tn.op)==LTYPE)&&(optype(r->tn.op)==LTYPE) 
			&& r->tn.op == l->tn.op && r->tn.name == l->tn.name
			&& r->tn.lval == l->tn.lval && r->tn.rval == l->tn.rval)
				{
				p->tn.op = ASSIGN;
				r->tn.op = FREE;
				r = bcon(0);
...
er:					p->tn.op = FREE;
	...
	and:				p->tn.op = FREE;
		...
		case MOD:
			/* 0 / a */
			if((l->tn.op == ICON && l->tn.lval == 0)
				|| (l->tn.op == FCON && l->fpn.dval == 0))
				goto and;
		case ASSIGN:
			if(((i = l->tn.op)==VAUTO || i == VPARAM || i == REG ||
			 i == TEMP || i == NAME)
			 && (r->in.op == PLUS || r->in.op == MINUS))
				{
				sp = r->in.left;
				if(sp->tn.op == i && r->in.right->tn.op==ICON &&
				!(ISPTR(r->in.right->tn.type)) &&
				r->in.right->tn.lval<17&&l->tn.lval==sp->tn.lval
				&& l->tn.rval == sp->tn.rval)
					{
					p->in.op = ASG r->in.op;
...
register NODE *p; 
{
	register NODE *q;
if( opdebug ) 
	printf("in optim p->tn.op=%s p->tn.type=0x%x\n",opst[p->tn.op],p->tn.type);
	if(((p->tn.op == ASG RS) || (p->tn.op == RS)) &&
		(p->in.right->tn.op != ICON)) {
		q = talloc();
		*q = *p;
		q->tn.op = UNARY MINUS;
		...
		p->in.right = q;
		p->tn.op += (LS - RS);
		if(p->tn.type==LONG || p->tn.type==ULONG) {
			q->in.left = makety(q->in.left,INT,0,INT);
			q->tn.type = INT;
			}
		if(p->tn.type==CHAR) q->tn.type =  SHORT;
		if(p->tn.type==UCHAR) q->tn.type = USHORT;
if( opdebug ) 
	printf("optim 2. p->tn.type = 0x%x q = 0x%x\n", p->tn.type,q->tn.type);
	}

	if((p->tn.op==ASG RS||p->tn.op==RS||p->tn.op==ASG LS||p->tn.op == LS)
		&&(p->in.right->tn.type!=INT)) {

		if(p->in.right->tn.op!=ICON
			&&(p->tn.type==LONG||p->tn.type==ULONG))
			p->in.right = makety(p->in.right,INT,0,INT);
		if( p->in.right->tn.op == ICON &&
		 (p->in.right->tn.type == CHAR||p->in.right->tn.type == UCHAR||
		 p->in.right->tn.type == LONG||p->in.right->tn.type == ULONG)){
			p->in.right->tn.type = INT;
		}
	}
