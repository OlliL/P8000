# include "mfile1.h"
#define  pdiff(i,j)  (((unsigned)(i) - (unsigned)(j)) / sizeof(*i))
/* da sonst pointerdiff fuer intasg. neg. werden, weil stab auf 2730 slots erhoeht */
...
# ifndef NODBG
	if( ddebug )
	{
		...
		tprint( q->in.type );
		printf( ", %s, (%d,%d) ), level %d, offset %ld\n",
		 scnames(class), q->fn.cdim, q->fn.csiz, blevel,p->offset );
	}
# endif
	fixtype( q, class );
	type = q->in.type;
...
mismatch:
	...
	if( class==MOU || class==MOS || class & FIELD )
	{
		...
		for( memp = &paramstk[paramno-1];
			/* while */ *memp>=0 && stab[(int)(*memp)].sclass != STNAME
				&& stab[(int)(*memp)].sclass != UNAME;
			/* iterate */ --memp)
		{
			if( stab[(int)(*memp)].sflags & SNONUNIQ )
			{
				if ( p->sname != stab[(int)(*memp)].sname )
					continue;
				lerror("redeclaration of: %s",
						p->sname);
				break;
			}
		}
	...
	lerror( "redeclaration of %s", p->sname );
...
# endif
	if( type == UNDEF ) lerror("void type for %s",p->sname);
	p->stype = type;
	p->sclass = class;
	p->slevel = blevel;
	p->offset = NOOFFSET;
...
# endif
	...
	if( p->slevel>1 && p->sclass == STATIC ) deflab((int) p->offset );
	else defnam( p );
	temp = p->stype;
	temp = DECREF( temp );
	ftntype = temp;
...
# ifndef NODBG
	if( ddebug>1 )
	{
		...
		{
			printf( "\tmember %s(%ld)\n", stab[(int)dimtab[i]].sname, dimtab[i] );
		}
	}
# endif
...
register idn;
{
	...
	case UNDEF:
		...
		break;
	case FARG:
		lerror("redeclaration of formal parameter, %s",p->sname);
		/* fall thru */
	case FTN:
...
getstr()
{
	    ...
	    pstk!=instack && ISARY( pstk[-1].in_t ) )
	{
		...
		*/
		...
		/* get the contents */
		lxstr((pstk-1)!=instack?(int)(dimtab[(pstk-1)->in_d]):0);
		irbrace();  /* simulate } */
		return( NIL );
	}
	else 
...
register NODE *pty; 
{
	...
	switch( (int)(new<0?pty->in.type:p->stype) )
	{
...
# endif
	...
	{
		if( ISARY(t) ) dstash( (int)(dimtab[i++]) );
	}
...
# ifndef NODBG
		if (ddebug)
			printf("removing %s = stab[ %d], flags %o level %d\n",
			    p->sname,pdiff(p,stab),p->sflags,p->slevel);
# endif
		if( p->sflags & SHIDES )
			unhide(p);
		if( (r=relook(p)) != p )
...
{
long diff;
diff = ((char *)p) - ((char *)stab);
diff &= 0177777L;
if(diff % sizeof(struct symtab)) goto err;
diff /= sizeof(struct symtab);
if(diff < 0 || diff >= SYMTSZ) {
	err: printf("SYMTSZ = %d,&stab = 0x%lx, size = %d, p = 0x%lx\n", SYMTSZ,(char *)stab, sizeof(struct symtab), (char *)p);
	fflush(stdout); }
}
