...
char yytext[LXTSZ];
char * lxgcp;
/* Let -T determine whether symbols are truncated. */

static int truncate_flag = 0;
		/* truncate based on -T (non-zero to truncate) */
...
char *argv[];
{
	...
	extern int zflag;
#endif
	extern unsigned long maxoffset;
	int fdef = 0;
...
yylex()
{
	for(;;)
	{
		...
		register char *cp;
		register id;
		p = lxcp[(lxchar = getchar())+1];
if(ddebug)
printf("yylex with Symbol %d (%c = %x)\n",p->lxact,lxchar,lxchar);
		switch( p->lxact )
		{
