...
/***** das hat tom herausgenommen ******
	n = (q->needs&NCOUNT);
	if( (q->needs&NPAIR) && n<HREG ) n = HREG;
****************************************/
	n = ((QCOUNT & q->needs) >> 8 )*4 +
	    ((PCOUNT & q->needs) >> 4 )*2 +
	      ( NCOUNT & q->needs) ;
...
# endif
			...
			nn += (rregs+2); 
				/* zwei sichert, dass rr0 nicht verg. wird*/
/****** Herausgenommen am 27.4 90 **********
			if (p->in.op == ASSIGN && p->in.type != TDOUBLE)
				nn +=2;
********************************************/
			if ( (p->in.type == TDOUBLE)  && 
				(QCOUNT & q->needs) ) nn -= 2; 
			if ( (p->in.op == CMP || p->in.op == CONV )  &&  
...
# ifndef NODBG
	if( odebug>1 ) {
		printf( "       matches %d, ", q->stinline);
		printf( "ls = %d", ls - shapes);
		printf( "(%s), ", ls? opst[ls->op]: "SHNL");
		printf( "rs = %d", rs - shapes);
		printf( "(%s),  s= ", rs? opst[rs->op]: "SHNL");
		pstrat( s );
		printf( "\n" );
		}
# endif
...
# ifndef NODBG
	if( e2debug>1 ) {
		printf( "\t\tfindsub( %d, %d )\n", p-node, s-shapes );
		printf( "op in tabelle = %s in knoten = %s\n",opst[s->op],
		opst[p->tn.op]);
		}
# endif
