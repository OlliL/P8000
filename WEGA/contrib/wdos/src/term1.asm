...
;**************************************************************************
;
;	P8000/WDOS-Terminal mit IBM-Tastatur
;
;**************************************************************************
;**************************************************************************
...
;---- Tastaturcode
;---- Normal-German (lower case)

TNGER:	defw	002bh, 011bh, 0231h, 0332h, 0433h, 0534h, 0635h, 0736h
	defw	0837h, 0938h, 0a39h, 0b30h, 0ce1h, 0d27h, 0e08h, 0f09h
	defw	1071h, 1177h, 1265h, 1372h, 1474h, 157ah, 1675h, 1769h
	defw	186fh, 1970h, 1a81h, 1b2bh, 1c0dh, ffffh, 1e61h, 1f73h
	defw	2064h, 2166h, 2267h, 2368h, 246ah, 256bh, 266ch, 2794h
	defw	2884h, 295eh, ffffh, 2b23h, 2c79h, 2d78h, 2e63h, 2f76h
	defw	3062h, 316eh, 326dh, 332ch, 342eh, 352dh, ffffh, 372ah
	defw	ffffh, 3920h, ffffh, 3b00h, 3c00h, 3d00h, 3e00h, 3f00h
	defw	4000h, 4100h, 4200h, 4300h, 4400h, ffffh, ffffh, 47e0h
	defw	48e0h, 49e0h, 4a2dh, 4be0h, 4cf0h, 4de0h, 4e3dh, 4fe0h
	defw	50e0h, 51e0h, 52e0h, 53e0h, ffffh, ffffh, 563ch, 8500h
	defw	8600h
	defw	1177h, 1372h
	defw	e02fh, e00dh
...
TSGER:	defw	002bh, 011bh, 0221h, 0322h, 0415h, 0524h, 0625h, 0726h
	defw	082fh, 0928h, 0a29h, 0b3dh, 0c3fh, 0d60h, 0e08h, 0f00h
	defw	1051h, 1157h, 1245h, 1352h, 1454h, 155ah, 1655h, 1749h
	defw	184fh, 1950h, 1a9ah, 1b2ah, 1c0dh, ffffh, 1e41h, 1f53h
	defw	2044h, 2146h, 2247h, 2348h, 244ah, 254bh, 264ch, 2799h
	defw	288eh, 29f8h, ffffh, 2b27h, 2c59h, 2d58h, 2e43h, 2f56h
	defw	3042h, 314eh, 324dh, 333bh, 343ah, 355fh, ffffh, 372ah
	...
	defw	5032h, 5133h, 5230h, 532ch, ffffh, ffffh, 563eh, 8700h
	defw	8800h
	defw	1157h, 1352h
	defw	e02fh, e00dh
...
TAASC:	defw	00f0h, 01f0h, 7800h, 7900h, 7a00h, 7b00h, 7c00h, 7d00h
...
TARASC:	defw	002bh, 011bh, ffffh, 03fdh, 04fch, ffffh, ffffh, ffffh
...
SOUT1:	cmp	ah,bh		;Attribute gleich ?
	...
	jz	SOUT2		;ja
	call	SATTR
SOUT2:	cmp	dx,184fh	;letzte	Zeile, letzte Spalte ?
	jz	CLREND		;ueber Clear to End ausgeben
				;damit Bildschirm nicht rollt
SOUT3:	inc	bp
SOUT0:	ret

SOUT4:	cmp	dx,184fh	;letzte	Zeile, letzte Spalte ?
	jnz	SOUT5		;nein
	call	ZOUT
	jmp short CLREND

SOUT5:	cmp	ax,bx		;neues = letztes Zeichen ?
	jz	SOUT3		;ja
...
ZOUT:	push	ax
	...
	cmp	al,0dh+1	;Diese Zeichen sind Steuerzeichen
	jc	ZOUT1		;und duerfen nicht ausgegeben werden
	cmp	al,1bh
	jz	ZOUT1
	cmp	al,1eh
	jnz	ZOUT2
...
ZOUT3:	call	PUTA		;Zeichen einzeln ausgeben
	dec	bp
	jnz	ZOUT3
	sub	bp,bp
	pop	ax
...
ZOUT4:	push	ax		;Zeichen ueber ESC-Folge ausgeben
	mov	al,1bh
	call	PUTA
	mov	al,'r'
	call	PUTA
	mov	ax,bp		;Anzahl
	add	al,20h		;plus Offset
	call	PUTA
	pop	ax
	call	PUTA
	sub	bp,bp
...
SATTR1:	cmp	ah,0fh		;normal intensiv
	jnz	SATTR2
	mov	al,38h
	jmp short PUTA

SATTR2:	and	ah,0f7h		;intensiv ausblenden
	cmp	ah,87h		;normal blinkend
	jnz	SATTR3
	mov	al,32h
	jmp short PUTA

SATTR3:	test	ah,80h		;blinkend ?
	jnz	SATTR4		;ja
	mov	al,34h		;invers
	jmp short PUTA

SATTR4:	mov	al,36h		;invers blinkend
