...
;**************************************************************************
;
;	BIOS Testversion fuer Erweiterungsmodule
;
;**************************************************************************
;**************************************************************************
...
I5:	MOV	WORD PTR [DI],0
	...
	CALL	DDS
	MOV	MEMORY_SIZE,704-64	;MEMORY SIZE SETZEN in KByte
...
;--- INT 19 ----------------------------------------------------
;BOOT DOS (hier ersetzt durch MP600-Test)
;---------------------------------------------------------------
MSGRUN:	defb	'test running',0dh,0ah

BOOT:	sti
BOOT0:	mov	SI,OFFSET MSGRUN
	call	P_MSG
	mov	cx,-1
	sub	ax,ax
	mov	ds,ax
	mov	bx,1000h
BOOT1:	mov	al,[bx]			;Byte gerade Adresse
	mov	dl,cs:[bx]
	mov	[bx],al
	mov	cs:[bx],bl
	inc	bx
	mov	al,[bx]			;Byte ungerade Adresse
	mov	dl,cs:[bx]
	mov	[bx],al
	mov	[bx],dl
	dec	bx
	mov	ax,[bx]			;Wort gerade Adresse
	mov	dx,cs:[bx]
	mov	[bx],ax
	mov	cs:[bx],dx
	inc	bx
	mov	ax,[bx]			;Wort ungerade Adresse
	mov	dx,cs:[bx]
	mov	[bx],ax
	mov	cs:[bx],dx
	dec	bx
