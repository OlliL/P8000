...
user_error:
	...
	if (AAsetusl(A_SLP, temptid, M_EXCL) < 0)
		goto tabort;
	if (AAsetrll(A_SLP, temptid, M_EXCL) < 0)
	{
		AAunlus(temptid);
tabort:
		i = 5000;
...
/*+*/	    !Reorg && 
# endif
	    close(creat(Mod_info.outfile, FILEMODE)))
...
/*+*/	else
# endif
		if (fd = fullwait(i, "modify"))	/* wait for db_sort */
			AAsyserr(11072, fd);
}
...
/*+*/		else
# endif
			if ((file.tup_fd = open(Mod_info.outfile, 0)) < 0)
...
#			endif
		...
		if (i && ((need > free) || (Fillfactor <= ((((long) used) * 100) / useable))))
		{
			used = 0;
			if (i = add_prim(&tid))
				AAsyserr(11080, i);
