...
unsigned ifno ;
{
	...
	if(nrifaces[ifno].bcstifle)
		return;	/* don't do it - K5JB */
...
struct mbuf *bp ;
{
	...
	char bcalias[7] ;
	struct nr3dest ds ;
	char sbuf[AXALEN*3] ;	/* not sure why this is * 3 -- K5JB */
...
char *neighbor ;			/* neighbor node + 3 digis (max)(K5JB) in arp format */
...
unsigned record ;			/* 1 if route is a "record route" */
{
	...
	if ((np = find_nrnbr(&ncall,ifnum)) == NULLNTAB) {
		if ((np =
			...
			memcpy(np->call,neighbor,(NRMAXDIGI + 1) * AXALEN) ;	/* ROSE mod - K5JB */
			np->interface = ifnum ;
			nhash = nrhash(&ncall) ;
		...
		memcpy(np->call,neighbor,(NRMAXDIGI + 1) * AXALEN) ; /* K5JB */
	}
