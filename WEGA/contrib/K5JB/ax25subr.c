/* modified del_ax25 to detect a second attempt to delete axp that happens
 * when a disconnect occurs when downloading a file over ax25 link.  There
 * probably needs to be a change somewhere in the state handler to prevent
 * this from happening but I haven't been able to find it.  This patch may
 * have undesirable side effects since axp is pointed to 0 in del_ax25.
 * K5JB 6/29/91
 * Undid the above.  Rewrote AX.25/Netrom mailbox to use transmit upcall.
 * Will eliminate the null assignment I added here with the define,
 * DOUBLETRAP.  7/15/91
 * Removed references to SID2. 3/12/91
 */
...
int32 t4init = 900000L / MSPTICK;	/* Disc Stale ckt in 15 minutes (900 sec) */
...
register struct ax25_cb *axp;
{
	...
	char i_state;
	int stop_timer();
	void free(),free_q(),restore();

	if(axp == NULLAX25)
		return;
	/* Remove from hash header list if first on chain */
	hashval = ax25hash(&axp->addr.dest);
	if(ax25_cb[hashval] == NULLAX25)	/* debugger shows this works - K5JB */
		return;
	i_state = disable();
	...
	free((char *)axp);
#ifdef DOUBLETRAP
	axp = NULLAX25;	/* try and prevent another attempt to free - K5JB */
#endif					/* Turned out to not be as good as fixing problem */
	restore(i_state);
}
 ...
 */
/* note that ssid is an unsigned char.  I didn't use that cast here for
 * fear of portability problems with some compilers.  This error seems
 * benign - K5JB */
...
char *call;
{
	...
	if(out == (struct ax25_addr *)0 || call == NULLCHAR || *call == '\0'){
		return -1;
	}
...
struct mbuf *data;
{
	...
	if(hdr == (struct ax25 *)0 || hdr->ndigis > MAXDIGIS)
		return NULLBUF;
