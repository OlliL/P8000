...
*									*
*	pb1.file.c	: wega p8000 16-bit-teil			*
*				c-quelle				*
*									*
************************************************************************/
...
extern int kenns, kennfo;
extern unsigned segnr, begadrf, begadre1, begadre2, entryf, lange1, lange2, langf, prlang;
...
unsigned nbytes;				/* read n bytes */
unsigned n_read;				/* tatsaechlich gelesene bytes */
unsigned n_write;				/* tatsaechlich geschriebene bytes */
...
{
int bwarning, beof;
	for (zhl2=0;zhl2<0x4000;zhl2++) progbuf[zhl2]=0xff;
	for (zhl2=0;zhl2<0x4000;zhl2++) eprombuf[zhl2]=0xff;
				...
				len=segt.sg_code+segt.sg_data;}
					...
					n_read=read (fd,peprombuf,nbytes);
						...
						switch (bre)
							{
							case 's': zhl1=1; break;
							case 'h': zhl1=2; nbytes=nbytes/2; break;
