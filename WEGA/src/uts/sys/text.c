...
register struct proc *p;
{
	...
	p->p_flag |= SLOCK;
	xccdec(p->p_textp);
	swap(swapdev, a, p->p_addr, os, B_WRITE);
	if (ff) 
...
loop:
	...
	ts = lbtoc(lc);
	xp->x_size = ts;
	if ((xp->x_daddr = malloc(swapmap, ctod((int)ts))) == NULL)
		panic("out of swap space");
	u.u_procp->p_textp = xp;
	xexpand(xp);
	u.u_segflg = 2;
	...
	u.u_segflg = 0;
	xp->x_flag = XWRIT;/* written into, must swap out */
}
...
register struct text *xp;
{
	if ((xp->x_caddr = malloc(coremap, xp->x_size)) != NULL) {
		if ((xp->x_flag&XLOAD)==0)
			swap(swapdev, xp->x_daddr, xp->x_caddr, xp->x_size, B_READ);
		xp->x_ccount++;
		xunlock(xp);
		return;
	}
...
register struct text *xp;
{
	...
	if (--xp->x_ccount==0) {
		if (xp->x_flag&XWRIT) {
			xp->x_flag &= ~XWRIT;
			swap(swapdev, xp->x_daddr, xp->x_caddr, xp->x_size, B_WRITE);
		}
		mfree(coremap, xp->x_size, xp->x_caddr);
	}
	xunlock(xp);
