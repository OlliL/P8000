...
*******************************************************************************
  ...
  U8000-Softwaremonitor (MON16)        Modul: p_test
...
!==============================================================================
Inhaltsuebersicht Modul p_test
------------------------------
Dieser Modul umfasst das Monitorkommando TEST (T).
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
  ...
  INTERNAL
T_TEST	WORD := %26
	ARRAY [* BYTE] := 'P8000 Hardwaretest U8001 - Version 3.1'
T_ERR	WORD := %000A
	ARRAY [* BYTE] :='*** ERROR '
T_FERR	WORD := %0010
	BYTE := BEL
...
++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++!
  ...
  GLOBAL
	...
	sc	#SC_WR_CRLF
	ld	r2, #T_TEST
	sc	#SC_WR_MSG	!Ausgabe Meldung!
	sc	#SC_WR_OUTBFF_CR !Ausgabe CR!
...
TSRAM5:
	...
	jr	z, TPIO		!ja --> naechster Testschritt!
...
--- Fehler 53, wenn beim Ruecklesen ein Fehler vom WDC gemeldet wurde bzw.
	...
	ld	ERR_WDC, #0	!Fehlerzaehler fuer WDC-Test loeschen!
	ld	r5, #%AAAA	!r5=Fehlerpar. (Testdatenwert)!
TWDC0:
	ld	r0, #2		!bei Fehler 52 Test 2x ausfuehren!
TWDC1:	
	push	@r15, r0
	...
	inc	ERR_WDC		!Fehlerzaehler WDC-Test erhoehen!
	pop	r5, @r15
	pop	r0, @r15
	djnz	r0, TWDC1
...
RAM_CHK:
	...
	jr	nz, T_LOOP	!nein --> naechstes Segment testen!
				...
				!ja!
	ld	r8, #%79	!Fehler 79: No Good Segments Above Zero!
	ld	ERRPAR_ID, #%0000 !Fehlerparameter fuer Fehler 79:
				   keine Parameter!
	calr	WR_TESTNR	!Ausgabe der Testschrittnummer!
...
!------------------------------------------------------
Testschritt 72, 73, 74, 76 (0100, 0101, 0102, 0103 'E')
RAM-Test Segment 0
------------------------------------------------------!

!
--- Fehler 72,73,74,76 - siehe RAM_TEST_SEG ---
!
...
RATE02:
	...
	ld	ERR_CNT, #0	!Fehlerzaehler loeschen 
				 (ERR_CNT liegt hier im hoechsten Segment)!
...
RATE04:
	...
	jp	@r10		!RUECKSPRUNG INS SEGMENT 0 ZU MMU_TEST!
    end RAM_TEST_SEG0
...
-------------------------------------------!
!
--- Fehler 84, wenn System-/Normal-Break-Register-Daten 'AA' oder '55' 
                 fehlerhaft sind ---
!
...
L1748:
	...
	test	REM_MMU1
	jr	z, LB_1A16
	ld	r2, #%4000	!rh2=logische Segmentnummer (64-127)!
LB_1A16:
...
RATE3:
	...
	ld	ERRPAR_ID, #%00EA !Fehlerparameter fuer Fehler 73, 74
							<r6>, r7, r5, r4 !
	andb	rh1, #%01	!Loeschen Bit1-7(rh1)!
...
RATE5:
	ld	r8, #%74	!Fehler 74: 'A' or '5' Data  Fault!
...
RATE6:
	...
	res	r9, #%0F
	ldctl	FCW, r9		!nichtsegmentierten Mode einstellen!
	calr	BY_WO_CMP	!Vergleich r4/r5; ggf. Fehler 74!
	cpb	rl5, #%55
	jr	z, RATE7	!naechster Testschritt!
...
RATE7:
	ld	r8, #%75	!Fehler 75: RAM-Paritaetsfehler!
	ld	ERRPAR_ID, #%00E8  !Fehlerparameter fuer Fehler 75:
				                                 <r6>, r7, r5!
	bitb	rh1, #0		!Test in Segment 0?!
	jr	z, RATE8	!nein!
	ld	r8, #%76	!Testschritt 76!
RATE8:
	andb	rh1, #%03	!Loeschen Bit2-7(rh1)!
	calr	WR_TESTNR	!Ausgabe der Testschrittnummer!

	!zu testendes Segment mit Monitormaschinencode %0000-%1FFF (8x)
	 fuellen!
	ld	r4, #8		!8*%2000=%10000 (1 Segment, 64k)!
	xor	r7, r7		!r7=Offset Zieladresse:= %0000
				 r6 enthaelt Segnr. Zieladresse!
RATE9:	
	ld	r12, #0         !r12=Segnr. Quelladresse := Segnr., wo Monitor!
	cp	r6, #0		!laueft ((MAX_SEGNR) bei Test im Segment 0,!
	jr	nz, RATE13	!sonst Segment 0)!
	ld	r12, MAX_SEGNR
RATE13:	ld	r13, #0		!r13=Offset Quelladresse := %0000!
	ld	r0, #%1000	!r0=WORD-Anzahl fuer ldir!
	set	r9, #%0F
	ldctl	FCW, r9		!segmentierten Mode einstellen!
	ldir	@r6, @r12, r0
	res	r9, #%0F
	...
	xor	r7, r7		!r7=Anfangs-Offset-Adresse fuer Vergleich:=%0!
	ld	REM_PARNMI, #0	!Merkerzelle 'Paritaets-NMI aufgetreten' 
				 loeschen!
	inb	rl0, S_BNK
	setb	rl0, #3		!Paritaetskontrolle einschalten!
	outb	S_BNK, rl0
RATE10:	set	r9, #%0F
	ldctl	FCW, r9		!segmentierten Mode einstellen!
	ld	r5, @r6		!Wert ruecklesen!
	res	r9, #%0F
	ldctl	FCW, r9		!nichtsegmentierten Mode einstellen!
	test	REM_PARNMI	!Paritaets-NMI aufgetreten?!
	jr	nz, RATE12	!ja, d.h. Paritaetsfehler!
	inc	r7, #2		!naechste Offsetadresse fuer Vergleich!
	test	r7		!ganzes Segment geprueft?!
	jr	nz, RATE10	!nein!
RATE11:
	inb	rl0, S_BNK
	resb	rl0, #3		!Paritaetskontrolle ausschalten!
	outb	S_BNK, rl0
	ret	

RATE12:				!Paritaetsfehler!
	ld	r4, r5		!r4 auf Wert ungleich r5 setzen,!
	inc	r4		!damit in BY_WO_CMP Fehler ausgegeben wird!
	calr	BY_WO_CMP	!Vergleich r4/r5; Fehler 75!
	ld	REM_PARNMI, #0	!Merkerzelle 'Paritaets-NMI aufgetreten'
				 zuruecksetzen!
	jr	RATE11
...
!
	ld	r2, #T_NSEG	Textausgabe 'NON-SEGMENTED'
	test	REM_MMU1
	jr	z, LB_2324
	ld	r2, #T_SEG	Textausgabe 'SEGMENTED'
LB_2324:
	sc	#SC_WR_MSG

	ld	r2, #T_JUM
	sc	#SC_WR_MSG	Textausgabe ' JUMPERS'
	sc	#SC_WR_OUTBFF_CR
!
	...
	add	r2, ERR_WDC	!Fehler von WDC-Test addieren!
	ret	
    end MSG_MAXSEG
...
(SIO0_B und CTC0_1 (vorher SIO0_A/SIO0_B und CTC0_0/CTC0_1))
...
!
	...
	testb	PTYPORT		!8-Bit-Teil aktiv?!
	ret	nz		!ja --> SIO0B nicht initialisieren!
!
